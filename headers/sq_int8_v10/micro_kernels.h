...
/* v9 fully-integer PatchEmbed LayerNorm fixed-point formats (MUST match
 * model_pytorch/weight_extractor_sq_int8_v9.py). F=12 frac bits on the dequantised input is implicit
 * in deq_mul/deq_sh and LN_EPSP=round(1e-5*2^(2F)). See layernorm_int_q.c. */
#define LN_QM    14U     /* rsqrt mantissa frac bits (m' in [1,4)) */
#define LN_QC    20U     /* gamma / pwl-coeff frac bits           */
#define LN_QG    20U     /* rsqrt mantissa output frac bits       */
#define LN_QN    20U     /* normalised-value frac bits            */
#define LN_QY    16U     /* affine down-shift target frac bits    */
#define LN_SEG   10U     /* rsqrt PWL segments (knot has SEG+1)    */
#define LN_EPSP  168     /* round(1e-5 * 2^24)                    */
#define GELU_LUT_ENTRIES 256U
...
/* <2. v8: fused int32 dequant + LayerNorm with range-reduced piece-wise-linear rsqrt.
 *     Reads the int32 conv accumulator + per-channel dequant scale, normalises over C, and
 *     approximates 1/sqrt(var+eps) with a LUT + linear interp (no sqrtf / no divide) - the
 *     FPGA/ASIC-friendly O(1) form. Only the rsqrt is approximated; mean/var/dequant match the
 *     f32 path. n_seg = PWL_RSQRT_SEGMENTS (knot has n_seg+1 entries). > */
void layernorm_int32_pwl_rsqrt(const int32_t *acc,
                    float *y,
                    const float *deq,
                    const float *gamma,
                    const float *beta,
                    uint32_t C, uint32_t L, float eps,
                    const float *pwl_knot,
                    const float *pwl_slope,
                    const float *pwl_intercept,
                    uint32_t n_seg);

/* <3. v9: fully-integer (fixed-point) PatchEmbed LayerNorm. int32 acc -> int8 (norm1, scale S_ln)
 *     or -> f32 (norm2). All mean/var/rsqrt/affine in int64; rsqrt = range-reduced PWL returning a
 *     full-precision (mantissa,exponent). See layernorm_int_q.c. nseg = LN_SEG. > */
...
/* <4. v9: int8 GELU via per-channel 256-entry LUT (fuses dequant+GELU+quant). In-place safe. > */
