...
void pvss_us_timing_f32(
    ...
    /* ----- Shape information ------------------------------- */
                ...
                uint32_t DW_CONV2D_KERNEL_SIZE, uint32_t MLP_HIDDEN_DIM,
                uint32_t SUB_D_INNER, uint32_t SUB_D_PROJECTION, // SUB_D_PROJECTION = 2 * SUB_D_INNER
                uint32_t SUB_C, uint32_t reduce_factor, uint16_t chunk_sz, uint32_t input_selection, uint32_t norm_ena, struct timespec *pvss_time, struct timespec *mlp_time);
