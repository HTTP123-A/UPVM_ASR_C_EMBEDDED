...
/* v8: number of segments in the piece-wise-linear approximation of rsqrt() used by the fused
 * int32 PatchEmbed LayerNorm (sources/sq_int8_v8/kernels/layernorm_int32_pwl_rsqrt.c). Range
 * reduction maps any variance to a mantissa in [1,4), so this one tuned segment count serves
 * every LayerNorm (the "max rule"); chosen by weight_extractor_sq_int8_v8.py as the min segment
 * count with rsqrt relative error <= 1.5e-3. If you re-tune to a different count, update this AND
 * re-run the extractor (the load-time file-size check enforces that they agree). */
#define PWL_RSQRT_SEGMENTS      10U
...
struct embed_weight {
    ...
    const float* norm2_w;
    const float* norm2_b;
    /* --- v8: shared PWL table approximating rsqrt(var+eps) for the fused int32 LayerNorm.
     *   Range-reduced (mantissa in [1,4)), hence data-independent: identical for norm1 & norm2 and
     *   for both streams. The conv1/conv2 dequant is FUSED into the norm (it reads the int32 conv
     *   accumulator + the per-channel conv deq_scale directly), so no separate f32 dequant runs. --- */
    const float* pwl_rsqrt_knot;        /* [PWL_RSQRT_SEGMENTS + 1] breakpoints on [1,4) */
    const float* pwl_rsqrt_slope;       /* [PWL_RSQRT_SEGMENTS]                          */
    const float* pwl_rsqrt_intercept;   /* [PWL_RSQRT_SEGMENTS]   (v8 f32 pwl; kept, unused by v9) */

    /* --- v9: fully-integer LayerNorm fixed-point tables (from weight_extractor_sq_int8_v9.py).
     *   norm1 -> int8 at calibrated S_ln (deq+gamma+output-quant+beta folded); norm2 -> f32.
     *   rsqrt_* is the shared int rsqrt PWL LUT; gelu_lut is per-channel [C_MID][256]. --- */
    const int32_t* norm1_deq_mul;   const int32_t* norm1_deq_sh;   const int32_t* norm1_gamma_q;
    const int32_t* norm1_amul;      const int32_t* norm1_ash;      const int32_t* norm1_bfix;
    const int32_t* norm2_deq_mul;   const int32_t* norm2_deq_sh;   const int32_t* norm2_gamma_q;
    const int32_t* rsqrt_knot_q;    const int32_t* rsqrt_sl_q;     const int32_t* rsqrt_ic_q;
    const int8_t*  gelu_lut;        /* [C_MID * 256] */
};
...
struct pvss_ds_weight {
    ...
    struct qconv_int8 ssm_dw_conv2d;
        /*< v5 REQUANT scale [SUB_D_INNER] = ssm_in_projection.deq_scale[:SUB_D_INNER] /
         *  ssm_dw_conv2d.act_scale; fuses the in_proj->dw_conv2d dequant->quant boundary on the
         *  x branch into one int32->int8 op (the z branch keeps its f32 dequant). >*/
    const float* ssm_requant_scale;
        /*< v7 INTEGER requant: ssm_requant_scale[c] ~= ssm_requant_mul[c] / 2^ssm_requant_shift[c]
         *  (dyadic multiplier, searched at export); used by requant_int32_to_int8_perchannel_fixed
         *  so the SSM x-branch requant runs integer-only. ssm_requant_scale kept for reference. >*/
    const int32_t* ssm_requant_mul;
    const int32_t* ssm_requant_shift;
    ...
    struct qconv_int8 mlp_fc2;
        /*< v4 REQUANT scale [MLP_HIDDEN_DIM] = mlp_fc1.deq_scale / mlp_fc2.act_scale;
         *  fuses the fc1->fc2 dequant->quant boundary into one int32->int8 op. >*/
    const float* mlp_requant_scale;
        /*< v6 INTEGER requant: mlp_requant_scale[c] ~= mlp_requant_mul[c] / 2^mlp_requant_shift[c]
         *  (dyadic multiplier, searched at export); used by requant_int32_to_int8_perchannel_fixed
         *  so the MLP requant runs with no f32 at runtime. mlp_requant_scale kept for reference. >*/
    const int32_t* mlp_requant_mul;
    const int32_t* mlp_requant_shift;
...
/* 1b. Weight file struct (int32 members; v6 fixed-point requant mul/shift) */
struct weight_file_spec_i32 {
    const char *member_name;
    size_t numel;
    const int32_t **slot;
};
