/* Timing variant of pvss_us_mp_int8 (sq_int8_v0, DETAIL profile).
 * Identical mixed-precision (f32 + int8) compute to model/pvss_us_mp_int8.c
 * (deterministic decoder: input offset + both norms always on); adds pvss_time
 * (SSM->MLP boundary) and mlp_time (function end, after PatchExpanding norm). */

#include "macro_kernels.h"
/*< 10. PVSS Block for Decoder - timing variant >*/
...
#pragma omp parallel num_threads((int)mp_parallel_factor)
{
    ...
    for (uint32_t g = g_start_idx; g < g_end_idx; g++) {
        /*< In-projection (int8): quant -> conv -> { requant x | dequant z } (v5) >*/
        {
            ...
            requant_int32_to_int8_perchannel(acc_slice, q_slice, SUB_D_INNER, L, pvss_us_weight->ssm_requant_scale);
    ...
    /*< MLP input projection (int8): [C,H,W] -> [MLP_HIDDEN_DIM,H,W] : quant -> conv -> relu -> requant >*/
    {
        ...
        requant_int32_to_int8_perchannel(acc_buf, quant_buf, MLP_HIDDEN_DIM, L, pvss_us_weight->mlp_requant_scale); /* v4: fused fc1 dequant + fc2 quant -> int8 (requant=deq/act_next) */
    }
