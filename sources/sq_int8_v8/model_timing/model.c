/* Split from sources/sq_int8_v0/model.c — STAGE_TIMING orchestrator.
 * Same mixed-precision (f32 + int8) forward as model/model.c
 * (dual_stream_unet_mp_int8); adds a struct timing_record_points* and a
 * clock_gettime sample at every stage boundary. Block calls are the NORMAL
 * int8 blocks (no per-block sub-timing — that is model_detail.c). */
