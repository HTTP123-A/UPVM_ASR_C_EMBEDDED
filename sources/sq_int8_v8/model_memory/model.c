...
static void log_patch_embed(const char *stage)
{
    ...
    /* v8: each conv's dequant is fused into the following LayerNorm; the norm reads int32 and uses
     * a shared PWL rsqrt table (knot[seg+1] + slope[seg] + intercept[seg], f32) plus per-channel
     * deq_scale on top of gamma+beta. */
    const uint64_t pwl_wb = (uint64_t)(2U * PWL_RSQRT_SEGMENTS + (PWL_RSQRT_SEGMENTS + 1U)) * 4U;
    ...
    mem_profile_log(stage, "norm1(LN-pwl)", "ACCEL->CPU", 1U, "int32", mid,
                    (uint64_t)3 * C_MID_PATCH_EMBED * 4U + pwl_wb, "f32", mid);
    f32op(stage, "gelu",      1U, mid, 0U, mid);
    qc_nodeq(stage, "conv2", 1U, mid, out, C_MID_PATCH_EMBED, C_OUT_PATCH_EMBED,
       (uint64_t)C_OUT_PATCH_EMBED * C_MID_PATCH_EMBED * K_SIZE_PATCH_EMBED * K_SIZE_PATCH_EMBED);
    mem_profile_log(stage, "norm2(LN-pwl)", "ACCEL->CPU", 1U, "int32", out,
                    (uint64_t)3 * C_OUT_PATCH_EMBED * 4U + pwl_wb, "f32", out);
}
