/* Mixed-precision (f32 + int8) VSS Output0 block for sq_int8_v0.
 * Output stage: the whole ss2d (in_proj / dw_conv2d / x_proj / dt / scan /
 * out_proj) runs f32; only mlp fc1 / fc2 and the sampler.expand (pe) run int8 as
 * an explicit quant -> int8 conv -> dequant sequence. Deterministic image of
 * output_layer_*.0: no block norms (Identity), input read from the skip-handler
 * offset (+C*L). */

#include "macro_kernels.h"
/*< 5. VSS Output0 - Pytorch equivalent: output_layer_*.0 (concat skip, no norm) >*/
...
{
    /* ---------------- SSM branch (f32) ---------------- */
    /*< Input is the skip-handler output (offset by C*L); no pre-norm (Identity) >*/
...
#pragma omp parallel num_threads((int)mp_parallel_factor)
{
    ...
    for (uint32_t g = g_start_idx; g < g_end_idx; g++) {
        ...
        pointwise_conv2d_f32(pvss_inout_buf_ptr + (size_t)g * SUB_C * L,
                            vss_output0_weight->ssm_in_projection_w,
                            NULL,
                            act_buf_slice_ptr,
        ...
        depthwise_conv2d_f32(act_buf_slice_ptr,
                            vss_output0_weight->ssm_dw_conv2d_w,
                            vss_output0_weight->ssm_dw_conv2d_b,
        ...
        grouped_pointwise_conv1d_type_major_f32(act_buf_slice_ptr + (size_t)4 * (SUB_D_INNER + R2N) * L,
                                                vss_output0_weight->ssm_dw_conv1d_w,
                                                NULL,
                                                act_buf_slice_ptr,
                                                SUB_D_INNER, 4, L, R, N);
        ...
        grouped_pointwise_conv1d_f32(act_buf_slice_ptr,
                                    vss_output0_weight->ssm_dt_projection_w,
                                    NULL,
        ...
        selective_scan_f32(act_buf_slice_ptr + (size_t)4 * (SUB_D_INNER + R2N) * L,
                            act_buf_slice_ptr + (size_t)4 * R2N * L,
                            vss_output0_weight->ssm_A,
                            act_buf_slice_ptr + (size_t)4 * R * L,
                            act_buf_slice_ptr + (size_t)4 * (R + N) * L,
                            vss_output0_weight->ssm_Ds,
                            vss_output0_weight->ssm_delta_bias,
        ...
        layernorm_f32(act_buf_slice_ptr + (size_t)4 * (SUB_D_INNER + R2N) * L,
                    vss_output0_weight->ssm_out_norm_w,
                    vss_output0_weight->ssm_out_norm_b,
                    SUB_D_INNER, L, 1e-5f);
        ...
        pointwise_conv2d_f32(act_buf_slice_ptr + (size_t)4 * (SUB_D_INNER + R2N) * L,
                            vss_output0_weight->ssm_out_projection_w,
                            NULL,
    ...
    /* ---------------- MLP branch ---------------- */
    /*< MLP Residual; no pre-norm (Identity) >*/
    ...
    {
        const struct qconv_int8 *q = &vss_output0_weight->mlp_fc1;
    ...
    /*< MLP ReLU >*/
    relu_f32(pvss_act_buf + (size_t)C * L, NULL, MLP_HIDDEN_DIM * L, 1);

    /*< MLP output projection (int8): [MLP_HIDDEN_DIM,H,W] -> [C,H,W] : quant -> conv -> dequant >*/
    {
        const struct qconv_int8 *q = &vss_output0_weight->mlp_fc2;
    ...
    {
        const struct qconv_int8 *q = &vss_output0_weight->pe;
    ...
    layernorm_f32(pvss_inout_buf,
                vss_output0_weight->pe_norm_w,
                vss_output0_weight->pe_norm_b,
                C_OUT, L_OUT, 1e-5f);
}
