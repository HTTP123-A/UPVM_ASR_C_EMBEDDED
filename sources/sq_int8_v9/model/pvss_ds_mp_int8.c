/* Mixed-precision (f32 + int8) PVSS downstream (encoder) block for sq_int8_v0.
 ...
 *              gated mul, residual add, mlp_sumpool, mlp_fc_res.
 * Per-group int8 scratch slices quant_buf/acc_buf by act_buf_slice_sz; the
 * sequential MLP uses the buffer base. */
    ...
    for (uint32_t g = g_start_idx; g < g_end_idx; g++) {
        /*< In-projection (int8): quant -> conv -> { requant x | dequant z } (v5) >*/
        {
            ...
            dequant_int32_to_f32_perchannel(acc_slice + (size_t)SUB_D_INNER * L,
                                            ...
                                            q->deq_scale + SUB_D_INNER); // For z chunk only
        }
