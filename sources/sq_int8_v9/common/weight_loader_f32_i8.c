...
/*< 6b. Load all int32 tensor files for one stage (v6 fixed-point requant mul/shift) >*/
static void load_i32_stage(const char *weight_root,
                    const char *stage_dir,
                    const struct weight_file_spec_i32 *specs,
                    size_t spec_count)
{
    char path_buf[WEIGHT_PATH_BUF_SIZE];
    size_t i;
    ...
    for (i = 0U; i < spec_count; ++i) {
        int32_t *buf = (int32_t *)alloc_bytes_or_die(specs[i].numel, sizeof(int32_t));
        ...
        read_file_exact(path_buf, buf, sizeof(int32_t), specs[i].numel);
        *(specs[i].slot) = buf;
    }
}
                            ...
                            uint32_t c_in, uint32_t c_mid, uint32_t c_out, uint32_t kernel_size)
{
    const struct weight_file_spec f32_specs[] = {
        ...
        /* v8: shared PWL rsqrt table for the fused int32 LayerNorm (same bytes in both streams). */
        {"pwl_rsqrt_knot",      (size_t)PWL_RSQRT_SEGMENTS + 1U, &weight->pwl_rsqrt_knot},
        {"pwl_rsqrt_slope",     (size_t)PWL_RSQRT_SEGMENTS,      &weight->pwl_rsqrt_slope},
        {"pwl_rsqrt_intercept", (size_t)PWL_RSQRT_SEGMENTS,      &weight->pwl_rsqrt_intercept},
    };
    ...
    /* v9: fully-integer LayerNorm fixed-point tables (int32). */
    const struct weight_file_spec_i32 i32_specs[] = {
        {"norm1_deq_mul",  (size_t)c_mid,        &weight->norm1_deq_mul},
        {"norm1_deq_sh",   1U,                   &weight->norm1_deq_sh},
        {"norm1_gamma_q",  (size_t)c_mid,        &weight->norm1_gamma_q},
        {"norm1_amul",     1U,                   &weight->norm1_amul},
        {"norm1_ash",      1U,                   &weight->norm1_ash},
        {"norm1_bfix",     (size_t)c_mid,        &weight->norm1_bfix},
        {"norm2_deq_mul",  (size_t)c_out,        &weight->norm2_deq_mul},
        {"norm2_deq_sh",   1U,                   &weight->norm2_deq_sh},
        {"norm2_gamma_q",  (size_t)c_out,        &weight->norm2_gamma_q},
        {"rsqrt_knot_q",   (size_t)LN_SEG + 1U,  &weight->rsqrt_knot_q},
        {"rsqrt_sl_q",     (size_t)LN_SEG,       &weight->rsqrt_sl_q},
        {"rsqrt_ic_q",     (size_t)LN_SEG,       &weight->rsqrt_ic_q},
    };
    ...
    load_i32_stage(weight_root, stage_dir, i32_specs, ARRAY_LEN(i32_specs));

    /* v9: per-channel int8 GELU LUT [c_mid][256]. */
    {
        char path_buf[WEIGHT_PATH_BUF_SIZE];
        const size_t n = (size_t)c_mid * GELU_LUT_ENTRIES;
        int8_t *buf = (int8_t *)alloc_bytes_or_die(n, sizeof(int8_t));
        build_weight_file_path(path_buf, sizeof(path_buf), weight_root, stage_dir, "gelu_lut");
        read_file_exact(path_buf, buf, sizeof(int8_t), n);
        weight->gelu_lut = buf;
    }
}
                            ...
                            uint32_t sub_c)
{
    ...
    const struct weight_file_spec_i32 i32_specs[] = {              /* v6 MLP + v7 SSM(x) fixed-point requant */
        {"mlp_requant_mul",   (size_t)mlp_hidden_dim, &weight->mlp_requant_mul},
        {"mlp_requant_shift", (size_t)mlp_hidden_dim, &weight->mlp_requant_shift},
        {"ssm_requant_mul",   (size_t)sub_d_inner,    &weight->ssm_requant_mul},
        {"ssm_requant_shift", (size_t)sub_d_inner,    &weight->ssm_requant_shift},
    };
