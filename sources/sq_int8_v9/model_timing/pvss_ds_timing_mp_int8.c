/* Timing variant of pvss_ds_mp_int8 (sq_int8_v0, DETAIL profile).
 * Identical mixed-precision (f32 + int8) compute to model/pvss_ds_mp_int8.c;
 * the only addition is two clock_gettime samples: pvss_time at the SSM->MLP
 * boundary and mlp_time at the function end, so the orchestrator can split
 * the encoder PVSS vs MLP cost. */
...
#pragma omp parallel num_threads((int)mp_parallel_factor)
{
    ...
    for (uint32_t g = g_start_idx; g < g_end_idx; g++) {
        ...
        {
            const struct qconv_int8 *q = &pvss_ds_weight->ssm_in_projection;
            ...
            requant_int32_to_int8_perchannel_fixed(acc_slice, q_slice, SUB_D_INNER, L, pvss_ds_weight->ssm_requant_mul, pvss_ds_weight->ssm_requant_shift); /* v7: integer fixed-point requant (mul/shift ~= deq/act_next); replaces v5 f32 SSM requant */
        ...
        {
            const struct qconv_int8 *q = &pvss_ds_weight->ssm_dw_conv2d;
        ...
        {
            const struct qconv_int8 *q = &pvss_ds_weight->ssm_out_projection;
    ...
    {
        const struct qconv_int8 *q = &pvss_ds_weight->mlp_fc1;
        ...
        depthwise_conv2d_int32(acc_buf, NULL, NULL, acc_buf, MLP_HIDDEN_DIM, H, W, 2U, 2U, 0U); /* int32 SumPool via depthwise conv (mlp_sumpool_w==1.0 => NULL unit taps), inplace on acc_buf 2x2 s2 -> [MLP_HIDDEN_DIM,H/2,W/2]; deq_scale>0 factors the scale out */
        requant_int32_to_int8_perchannel_fixed(acc_buf, quant_buf, MLP_HIDDEN_DIM, L2, pvss_ds_weight->mlp_requant_mul, pvss_ds_weight->mlp_requant_shift); /* v6: integer fixed-point requant (mul/shift ~= deq/act_next); replaces v4 f32 requant */
    }

    /*< MLP res sumpool (f32): [C, H, W] -> [C, H/2, W/2] >*/
    ...
    /*< MLP output projection (int8): [MLP_HIDDEN_DIM,H/2,W/2] -> [2C,H/2,W/2] : conv -> dequant (quant folded into fc1's requant) >*/
    {
        const struct qconv_int8 *q = &pvss_ds_weight->mlp_fc2;
        pointwise_conv2d_int8(quant_buf, q->w_i8, q->bias_i32, acc_buf, MLP_HIDDEN_DIM, H2, W2, C2);
        dequant_int32_to_f32_perchannel(acc_buf, pvss_inout_buf, C2, L2, q->deq_scale);
    }

    /*< MLP res output projection (f32): [C, H/2, W/2] -> [2C, H/2, W/2] >*/
