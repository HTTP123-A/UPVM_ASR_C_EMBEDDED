...
/*< v8: int8 layer whose dequant is FUSED into the next op (the PatchEmbed PWL LayerNorm):
 *  quant + int8 conv only - the int32 accumulator is drained by the norm, not a separate dequant. >*/
static void qc_nodeq(const char *stage, const char *base, uint32_t rep,
               uint64_t in_elems, uint64_t out_elems,
               uint32_t c_in, uint32_t c_out, uint64_t w_i8)
{
    char nm[96];
                          ...
                          uint32_t R2N, uint32_t red, uint32_t dwk)
{
    ...
    /* in_proj (v5): quant -> int8 conv [2D] -> { requant x[D]:int32->int8 | dequant z[D]:int32->f32 }.
     * The dw_conv2d quant is folded into the x-branch requant (requant_scale = in_proj.deq / dw_conv.act). */
    const uint64_t in_wb = (uint64_t)(2 * D) * SUB_C + (uint64_t)(2 * D) * 4U + (uint64_t)SUB_C * 4U; /* w_i8 + deq_scale[2D] + act_scale[SUB_C]; in_proj has no bias */
    mem_profile_log(stage, "in_proj.quant",     "CPU->ACCEL", red, "f32",   (uint64_t)SUB_C * Ll, 0U,             "int8",  (uint64_t)SUB_C * Ll);
    mem_profile_log(stage, "in_proj.int8conv",  "INT8-ACCEL", red, "int8",  (uint64_t)SUB_C * Ll, in_wb,          "int32", (uint64_t)(2 * D) * Ll);
    mem_profile_log(stage, "in_proj.requant_x", "INT8-ACCEL", red, "int32", (uint64_t)D * Ll,     (uint64_t)D * 8U, "int8",  (uint64_t)D * Ll); /* v7: integer fixed-point requant, mul[D]+shift[D] int32 (x branch) */
    mem_profile_log(stage, "in_proj.dequant_z", "ACCEL->CPU", red, "int32", (uint64_t)D * Ll,     0U,             "f32",   (uint64_t)D * Ll);  /* z branch stays f32 */
    /* dw_conv2d (v5): int8 conv -> relu_x(int32, logged in core) -> dequant; quant folded into in_proj.requant_x */
    const uint64_t dw_wb = (uint64_t)D * dwk * dwk + (uint64_t)D * 4U + (uint64_t)D * 4U; /* w_i8 + bias[D] + deq_scale[D] */
    mem_profile_log(stage, "dw_conv2d.int8conv", "INT8-ACCEL", red, "int8",  (uint64_t)D * Ll, dw_wb, "int32", (uint64_t)D * Ll);
    mem_profile_log(stage, "dw_conv2d.dequant",  "ACCEL->CPU", red, "int32", (uint64_t)D * Ll, 0U,    "f32",   (uint64_t)D * Ll);
...
static void log_mlp_ds(const char *stage, uint32_t C, uint32_t L, uint32_t MLPh)
{
    ...
    mem_profile_log(stage, "mlp_fc1.requant",    "INT8-ACCEL", 1U, "int32", (uint64_t)MLPh * L2, (uint64_t)MLPh * 8U, "int8", (uint64_t)MLPh * L2); /* v6: integer fixed-point requant, mul[MLPh]+shift[MLPh] int32 */
...
static void log_mlp_plain(const char *stage, uint32_t C, uint32_t L, uint32_t MLPh, uint32_t has_norm, uint32_t fused)
{
    ...
    if (fused) {
        ...
        mem_profile_log(stage, "mlp_fc1.int8conv", "INT8-ACCEL", 1U, "int8",  (uint64_t)C * Ll,    fc1_wb, "int32", (uint64_t)MLPh * Ll);
        mem_profile_log(stage, "mlp_relu(int32)",  "INT8-ACCEL", 1U, "int32", (uint64_t)MLPh * Ll, 0U,     "int32", (uint64_t)MLPh * Ll);
        mem_profile_log(stage, "mlp_fc1.requant",  "INT8-ACCEL", 1U, "int32", (uint64_t)MLPh * Ll, (uint64_t)MLPh * 8U, "int8", (uint64_t)MLPh * Ll); /* v6: integer fixed-point requant, mul[MLPh]+shift[MLPh] int32 */
        /* fc2 (v4): int8 conv -> dequant (quant folded into fc1's requant; no act_scale read) */
...
static void log_patch_embed(const char *stage)
{
    ...
    /* v9: fully-integer LayerNorm (norm1 int32->int8, norm2 int32->f32) + per-channel int8 GELU LUT;
     * conv2 reads the GELU int8 directly (no quant). Norm weights = int32 deq_mul+gamma_q(+bfix) +
     * the shared int rsqrt LUT (knot[seg+1]+slope[seg]+icpt[seg], int32); norm2's beta stays f32. */
    const uint64_t rs_wb = (uint64_t)(2U * LN_SEG + (LN_SEG + 1U)) * 4U;
    const uint64_t n1_wb = (uint64_t)3U * C_MID_PATCH_EMBED * 4U + rs_wb;                 /* deq_mul+gamma_q+bfix */
    const uint64_t n2_wb = (uint64_t)3U * C_OUT_PATCH_EMBED * 4U + rs_wb;                 /* deq_mul+gamma_q(i32)+beta(f32) */
    qc_nodeq(stage, "conv1", 1U, in1, mid, C_IN_PATCH_EMBED, C_MID_PATCH_EMBED,
       (uint64_t)C_MID_PATCH_EMBED * C_IN_PATCH_EMBED * K_SIZE_PATCH_EMBED * K_SIZE_PATCH_EMBED);
    mem_profile_log(stage, "norm1(I-LN)",    "ACCEL->CPU", 1U, "int32", mid, n1_wb, "int8", mid);
    mem_profile_log(stage, "gelu(int8-lut)", "INT8-ACCEL", 1U, "int8",  mid,
                    (uint64_t)C_MID_PATCH_EMBED * 256U, "int8", mid);
    mem_profile_log(stage, "conv2.int8conv", "INT8-ACCEL", 1U, "int8",  mid,
                    (uint64_t)C_OUT_PATCH_EMBED * C_MID_PATCH_EMBED * K_SIZE_PATCH_EMBED * K_SIZE_PATCH_EMBED
                    + (uint64_t)C_OUT_PATCH_EMBED * 4U + ((uint64_t)C_MID_PATCH_EMBED + C_OUT_PATCH_EMBED) * 4U,
                    "int32", out);
    mem_profile_log(stage, "norm2(I-LN)",    "CPU-F32",    1U, "int32", out, n2_wb, "f32",  out);
}
