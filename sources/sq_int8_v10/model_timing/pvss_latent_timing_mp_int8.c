/* Timing variant of pvss_latent_mp_int8 (sq_int8_v0, DETAIL profile).
 * Identical mixed-precision (f32 + int8) compute to model/pvss_latent_mp_int8.c;
 * adds pvss_time (SSM->MLP boundary) and mlp_time (function end) only. */
...
#pragma omp parallel num_threads((int)mp_parallel_factor)
{
    ...
    for (uint32_t g = g_start_idx; g < g_end_idx; g++) {
        /*< In-projection (int8): quant -> conv -> { requant x | dequant z } (v5) >*/
        {
            ...
            requant_int32_to_int8_perchannel_fixed(acc_slice, q_slice, SUB_D_INNER, L, pvss_latent_weight->ssm_requant_mul, pvss_latent_weight->ssm_requant_shift); /* v7: integer fixed-point requant (mul/shift ~= deq/act_next); replaces v5 f32 SSM requant */
    ...
    /*< MLP input projection (int8): [C,H,W] -> [MLP_HIDDEN_DIM,H,W] : quant -> conv -> relu -> requant >*/
    {
        ...
        requant_int32_to_int8_perchannel_fixed(acc_buf, quant_buf, MLP_HIDDEN_DIM, L, pvss_latent_weight->mlp_requant_mul, pvss_latent_weight->mlp_requant_shift); /* v6: integer fixed-point requant (mul/shift ~= deq/act_next); replaces v4 f32 requant */
    }
