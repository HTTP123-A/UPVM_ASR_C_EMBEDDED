...
#include "model.h" 
void dual_stream_unet_timing_profiling_f32(float* audio, // [L_AUDIO]
    /* ----- weights ----------------------------------------- */
                        ...
                        struct timing_record_points *timing_record)
{
    /*< ---------------------------------------------------------------------------------------------------------------------- >*/
    /* --- [1] Get start time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->start_time));
    ...
    stft_f32(audio, model_inout_buf_mag, model_inout_buf_pha,
            ...
            /* --- [2] Get stft time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->stft_time));

        /*< Copy low freq >*/
    memcpy(model_low_freq_buf_mag, model_inout_buf_mag, LOW_FREQ_ELEMENT * sizeof(float)); // Copy from 0th to HF * 512 elements
            /* --- [3] Get mag low frequency time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->low_frequency_mag_time));    

    memcpy(model_low_freq_buf_pha, model_inout_buf_pha, LOW_FREQ_ELEMENT * sizeof(float));
            /* --- [4] Get pha low frequency time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->low_frequency_pha_time));
    ...
    memcpy(model_global_res_buf_mag + (size_t)SKIP_OFFSET_SPECTROGRAM, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, C_IN_PATCH_EMBED * L_IN_PATCH_EMBED * sizeof(float));    
            /* --- [5] Get spectrogram residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->spectrogram_res_mag_time));
    ...
    patch_embed_f32(model_inout_buf_mag  + (size_t)W_IN_PATCH_EMBED, // Similar, reserved W_IN_PATCH_EMBED elements for 1st freq
            ...
            /* --- [6] Get patch embed mag time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->patch_embed_mag_time));
    ...
    patch_embed_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                ...
                H_IN_PATCH_EMBED, W_IN_PATCH_EMBED, K_SIZE_PATCH_EMBED,
                STRIDE_PATCH_EMBED, PAD_PATCH_EMBED);
            /* --- [7] Get patch embed pha time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->patch_embed_pha_time));
    ...
    memcpy(model_global_res_buf_mag + (size_t)SKIP_OFFSET_PATCH_EMBED, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, C_ENC_0 * L_ENC_0 * sizeof(float));
            /* --- [8] Get patch embed mag residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->patch_embed_res_mag_time));
    ...
    memcpy(model_global_res_buf_pha + (size_t)SKIP_OFFSET_PATCH_EMBED - SKIP_OFFSET_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, C_ENC_0 * L_ENC_0 * sizeof(float));
            /* --- [9] Get patch embed pha residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->patch_embed_res_pha_time));
    ...
    elemwise_add_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, (size_t)C_ENC_0 * L_ENC_0); // mag = mag + phase
            /* --- [10] Get patch embed mag interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->patch_embed_interaction_mag_time));
    ...
    elemwise_add_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, (size_t)C_ENC_0 * L_ENC_0); // phase = phase + mag
            /* --- [11] Get patch embed pha interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->patch_embed_interaction_pha_time));


    /*< --- Encoder 0 --- >*/
        /*< Mag >*/
    pvss_ds_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [12] Get encoder 0 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder0_mag_time));

        /*< Pha >*/
    pvss_ds_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
            ...
            SUB_D_INNER_ENC_0, SUB_D_PROJECTION_ENC_0,
            SUB_C_ENC_0, REDUCTION_FACTOR_ENC_0, CHUNK_SZ_ENC_0);
            /* --- [13] Get encoder 0 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder0_pha_time));
    ...
    memcpy(model_global_res_buf_mag + (size_t)SKIP_OFFSET_ENC_0, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, C_ENC_1 * L_ENC_1 * sizeof(float));
            /* --- [14] Get patch embed mag residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder0_res_mag_time));
    ...
    memcpy(model_global_res_buf_pha + (size_t)SKIP_OFFSET_ENC_0 - SKIP_OFFSET_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, C_ENC_1 * L_ENC_1 * sizeof(float));
            /* --- [15] Get patch embed pha residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder0_res_pha_time));
    ...
    elemwise_add_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, (size_t)C_ENC_1 * L_ENC_1); // mag = mag + phase
            /* --- [16] Get patch embed mag interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder0_interaction_mag_time));
    ...
    elemwise_add_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, (size_t)C_ENC_1 * L_ENC_1); // phase = phase + mag
            /* --- [17] Get patch embed pha interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder0_interaction_pha_time));
    ...
    pvss_ds_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [18] Get encoder 1 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder1_mag_time));
    ...
    pvss_ds_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
            ...
            SUB_D_INNER_ENC_1, SUB_D_PROJECTION_ENC_1,
            SUB_C_ENC_1, REDUCTION_FACTOR_ENC_1, CHUNK_SZ_ENC_1);
            /* --- [19] Get encoder 1 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder1_pha_time));
    ...
    memcpy(model_global_res_buf_mag + (size_t)SKIP_OFFSET_ENC_1, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, C_ENC_2 * L_ENC_2 * sizeof(float));
            /* --- [20] Get patch embed mag residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder1_res_mag_time));
    ...
    memcpy(model_global_res_buf_pha + (size_t)SKIP_OFFSET_ENC_1 - SKIP_OFFSET_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, C_ENC_2 * L_ENC_2 * sizeof(float));
            /* --- [21] Get patch embed pha residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder1_res_pha_time));
    ...
    elemwise_add_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, (size_t)C_ENC_2 * L_ENC_2); // mag = mag + phase
            /* --- [22] Get patch embed mag interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder1_interaction_mag_time));
    ...
    elemwise_add_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, (size_t)C_ENC_2 * L_ENC_2); // phase = phase + mag
            /* --- [23] Get patch embed pha interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder1_interaction_pha_time));
    ...
    pvss_ds_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [24] Get encoder 2 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder2_mag_time));
    ...
    pvss_ds_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
            ...
            SUB_D_INNER_ENC_2, SUB_D_PROJECTION_ENC_2,
            SUB_C_ENC_2, REDUCTION_FACTOR_ENC_2, CHUNK_SZ_ENC_2);
            /* --- [25] Get encoder 2 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder2_pha_time));
    ...
    memcpy(model_global_res_buf_mag + (size_t)SKIP_OFFSET_ENC_2, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, C_LATENT * L_LATENT * sizeof(float));
            /* --- [26] Get patch embed mag residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder2_res_mag_time));
    ...
    memcpy(model_global_res_buf_pha + (size_t)SKIP_OFFSET_ENC_2 - SKIP_OFFSET_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, C_LATENT * L_LATENT * sizeof(float));
            /* --- [27] Get patch embed pha residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder2_res_pha_time));
    ...
    elemwise_add_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, (size_t)C_LATENT * L_LATENT); // mag = mag + phase
            /* --- [28] Get patch embed mag interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder2_interaction_mag_time));
    ...
    elemwise_add_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, (size_t)C_LATENT * L_LATENT); // phase = phase + mag
            /* --- [29] Get patch embed mag interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->encoder2_interaction_pha_time));

    /*< ---------------------------------------------------------------------------------------------------------------------- >*/
    /*< --- Latent ------ >*/
        /*< Mag >*/
    pvss_latent_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
                    model_internal_res_buf_mag,
                    model_act_buf_mag,
                    hidden_state_buf_mag,
            ...
            /* --- [30] Get latent pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->latent_mag_time));

        /*< Pha >*/
    pvss_latent_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                    model_internal_res_buf_pha,
                    model_act_buf_pha,
                    hidden_state_buf_pha,
                    ...
                    SUB_D_INNER_LATENT, SUB_D_PROJECTION_LATENT,
                    SUB_C_LATENT, REDUCTION_FACTOR_LATENT, CHUNK_SZ_LATENT);
            /* --- [31] Get latent pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->latent_pha_time));

        /*< Interaction >*/
    elemwise_add_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, (size_t)C_LATENT * L_LATENT); // mag = mag + phase
            /* --- [32] Get latent interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->latent_interaction_mag_time));

    elemwise_add_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, (size_t)C_LATENT * L_LATENT); // phase = phase + mag
            /* --- [33] Get latent interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->latent_interaction_pha_time));
    ...
    pointwise_conv2d_split2_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [34] Get decoder 0 residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder0_res_mag_time));
    ...
    pointwise_conv2d_split2_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                            ...
                            C_DEC_0, H_DEC_0, W_DEC_0, C_DEC_0);
            /* --- [35] Get decoder 0 residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder0_res_pha_time));

        /*< Mag >*/
    pvss_us_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
                model_internal_res_buf_mag,
                model_act_buf_mag,
                hidden_state_buf_mag,
            ...
            /* --- [36] Get decoder 0 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder0_mag_time));

        /*< Pha >*/
    pvss_us_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                model_internal_res_buf_pha,
                model_act_buf_pha,
                hidden_state_buf_pha,
                ...
                SUB_D_INNER_DEC_0, SUB_D_PROJECTION_DEC_0,
                SUB_C_DEC_0, REDUCTION_FACTOR_DEC_0, CHUNK_SZ_DEC_0, 1, 1);
            /* --- [37] Get decoder 0 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder0_pha_time));
    ...
    elemwise_add_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, (size_t)C_DEC_1 * L_DEC_1); // mag = mag + phase
            /* --- [38] Get decoder 0 interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder0_interaction_mag_time));
    ...
    elemwise_add_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, (size_t)C_DEC_1 * L_DEC_1); // phase = phase + mag
            /* --- [39] Get decoder 0 interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder0_interaction_pha_time));
    ...
    pointwise_conv2d_split2_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [40] Get decoder 1 residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder1_res_mag_time));
    ...
    pointwise_conv2d_split2_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                            ...
                            C_DEC_1, H_DEC_1, W_DEC_1, C_DEC_1);
            /* --- [41] Get decoder 1 residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder1_res_pha_time));
    ...
    pvss_us_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [42] Get decoder 1 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder1_mag_time));
    ...
    pvss_us_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                ...
                SUB_D_INNER_DEC_1, SUB_D_PROJECTION_DEC_1,
                SUB_C_DEC_1, REDUCTION_FACTOR_DEC_1, CHUNK_SZ_DEC_1, 1, 1);
            /* --- [43] Get decoder 1 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder1_pha_time));
    ...
    elemwise_add_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, (size_t)C_DEC_2 * L_DEC_2); // mag = mag + phase
            /* --- [44] Get decoder 1 interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder1_interaction_mag_time));
    ...
    elemwise_add_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, (size_t)C_DEC_2 * L_DEC_2); // phase = phase + mag
            /* --- [45] Get decoder 1 interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder1_interaction_pha_time));
    ...
    pointwise_conv2d_split2_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [46] Get decoder 2 residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder2_res_mag_time));
    ...
    pointwise_conv2d_split2_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                            ...
                            C_DEC_2, H_DEC_2, W_DEC_2, C_DEC_2);
            /* --- [47] Get decoder 2 residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder2_res_pha_time));
    ...
    pvss_us_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [48] Get decoder 2 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder2_mag_time));
    ...
    pvss_us_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                ...
                SUB_D_INNER_DEC_2, SUB_D_PROJECTION_DEC_2,
                SUB_C_DEC_2, REDUCTION_FACTOR_DEC_2, CHUNK_SZ_DEC_2, 1, 1);
            /* --- [49] Get decoder 2 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder2_pha_time));
    ...
    elemwise_add_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, (size_t)C_OUT_0 * L_OUT_0); // mag = mag + phase
            /* --- [50] Get decoder 2 interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder2_interaction_mag_time));
    ...
    elemwise_add_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED, model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, (size_t)C_OUT_0 * L_OUT_0); // phase = phase + mag
            /* --- [51] Get decoder 2 interaction time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->decoder2_interaction_pha_time));
    ...
    pointwise_conv2d_split2_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [52] Get output 0 residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->output0_res_mag_time));
    ...
    pointwise_conv2d_split2_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                            ...
                            C_OUT_0, H_OUT_0, W_OUT_0, C_OUT_0);
            /* --- [53] Get output 0 residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->output0_res_pha_time));
    ...
    pvss_us_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [54] Get output 0 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->output0_mag_time));
    ...
    pvss_us_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                ...
                SUB_D_INNER_OUT_0, SUB_D_PROJECTION_OUT_0,
                SUB_C_OUT_0, REDUCTION_FACTOR_OUT_0, CHUNK_SZ_OUT_0, 1, 0);
            /* --- [55] Get output 0 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->output0_pha_time));
    ...
    pvss_us_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
            ...
            /* --- [56] Get output 1 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->output1_mag_time));
    ...
    pvss_us_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                ...
                SUB_D_INNER_OUT_1, SUB_D_PROJECTION_OUT_1,
                SUB_C_OUT_1, REDUCTION_FACTOR_OUT_1, CHUNK_SZ_OUT_1, 0, 1);
            /* --- [57] Get output 1 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->output1_pha_time));

    /*< --- Output 3 ---- >*/
        /*< Mag >*/
    vss_output3_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED,
                    model_internal_res_buf_mag,
                    model_act_buf_mag,
                    hidden_state_buf_mag,
            ...
            /* --- [58] Get output 1 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->output3_mag_time));

        /*< Pha >*/
    vss_output3_f32(model_inout_buf_pha + (size_t)W_IN_PATCH_EMBED,
                    model_internal_res_buf_pha,
                    model_act_buf_pha,
                    hidden_state_buf_pha,
                    ...
                    DW_KERNEL_SZ_OUT_3, MLP_HIDDEN_OUT_3,
                    D_INNER_OUT_3, D_PROJECTION_OUT_3, CHUNK_SZ_OUT_3);
            /* --- [59] Get output 1 pvss time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->output3_pha_time));
    ...
    elemwise_add_f32(model_inout_buf_mag + (size_t)W_IN_PATCH_EMBED, model_global_res_buf_mag + (size_t)SKIP_OFFSET_SPECTROGRAM, (size_t)C_IN_PATCH_EMBED * L_IN_PATCH_EMBED); // mag = mag + residual (spectrogram)
            /* --- [60] Get output 3 residual time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->output3_res_mag_time));
    ...
    memcpy(model_inout_buf_mag, model_low_freq_buf_mag, LOW_FREQ_ELEMENT * sizeof(float)); // Copy back low freq to the 0th ~ HF * 512 elements
            /* --- [61] Get final low-freq replacement time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->final_low_freq_mag_time));

    memcpy(model_inout_buf_pha, model_low_freq_buf_pha, LOW_FREQ_ELEMENT * sizeof(float));
            /* --- [62] Get final low-freq replacement time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->final_low_freq_pha_time));
    ...
    istft_f32(model_inout_buf_mag, model_inout_buf_pha, audio,
            ...
            /* --- [63] Get iSTFT time ---*/
    clock_gettime(CLOCK_MONOTONIC, &(timing_record->istft_time));
}
