                          ...
                          uint32_t R2N, uint32_t red, uint32_t dwk)
{
    ...
    log_ss2d_core_f32(stage, L, D, R, N, R2N, red);
...
static void log_mlp_ds(const char *stage, uint32_t C, uint32_t L, uint32_t MLPh)
{
    ...
    /* fc1 (v2): quant -> int8 conv -> relu_int32 -> SumPool(int32) -> dequant(compacted L/4) */
    const uint64_t fc1_wb = (uint64_t)MLPh * C + (uint64_t)MLPh * 4U + ((uint64_t)C + MLPh) * 4U;
    ...
    mem_profile_log(stage, "mlp_fc1.dequant",    "ACCEL->CPU", 1U, "int32", (uint64_t)MLPh * L2, 0U,     "f32",   (uint64_t)MLPh * L2);
    f32op(stage, "mlp_sumpool_res(dwconv)", 1U, (uint64_t)C * Ll, (uint64_t)C * 4U * 4U, (uint64_t)C * L2);
    qc(stage, "mlp_fc2", 1U, (uint64_t)MLPh * L2, (uint64_t)C2 * L2, MLPh, C2, (uint64_t)C2 * MLPh);
...
static void log_mlp_plain(const char *stage, uint32_t C, uint32_t L, uint32_t MLPh, uint32_t has_norm)
{
    const uint64_t Ll = L;
    f32op(stage, "mlp_residual_copy", 1U, (uint64_t)C * Ll, 0U, (uint64_t)C * Ll);
    if (has_norm) {
...
static void log_output3(const char *stage)
{
    ...
    log_ss2d_core_f32(stage, L_OUT_3, D, R, N, R2N, 1U);
                        ...
                        const char *csv_path)
{
    ...
    log_mlp_plain("latent", C_LATENT, L_LATENT, MLP_HIDDEN_LATENT, 1U);
    ...
    log_mlp_plain("dec0", C_DEC_0, L_DEC_0, MLP_HIDDEN_DEC_0, 1U);
    ...
    log_mlp_plain("dec1", C_DEC_1, L_DEC_1, MLP_HIDDEN_DEC_1, 1U);
    ...
    log_mlp_plain("dec2", C_DEC_2, L_DEC_2, MLP_HIDDEN_DEC_2, 1U);
    ...
    log_mlp_plain("out0", C_OUT_0, L_OUT_0, MLP_HIDDEN_OUT_0, 0U);
    ...
    log_mlp_plain("out1", C_OUT_1, L_OUT_1, MLP_HIDDEN_OUT_1, 1U);
