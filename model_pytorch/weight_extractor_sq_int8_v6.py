#!/usr/bin/env python3
"""Weight extractor for the sq_int8_v6 INT8-F32 mixed-precision C model.

v6 = v5 weights PLUS, for every fused MLP stage, an INTEGER fixed-point requant pair
that lets the MLP `fc1 -> fc2` REQUANT run with no floating point at runtime:

    mlp_requant_scale[c]  (f32, from v4)   ~=   mlp_requant_mul[c] / 2^mlp_requant_shift[c]
...
sources/sq_int8_v6/kernels/requant_int32_to_int8_perchannel_fixed.c):
...
ssm_requant_scale artifact (so all of those are byte-identical to v5); v6 only ADDS the 14
mlp_requant_mul.bin + 14 mlp_requant_shift.bin (int32) files.

Source checkpoint: model/gen_ptq_int8pack.pth
Output:            ../data/weight_sq_int8_v6/<stage_dir>/<member>.bin (+ meta/manifest)
"""
...
OUTPUT_ROOT = HERE.parent / "data" / "weight_sq_int8_v6"
...
def _build_mlp_requant_fixed(mlp_extra):
    """From v4's mlp_requant_scale records, derive per-channel integer (mul, shift).
    Returns (extra_field_records, triples) where triples = (stage, scale, mul, shift)."""
    extra, triples = [], []
    for field_name, scale, _src, _note in mlp_extra:
        ...
        extra.append((f"{stage}.mlp_requant_mul", np.ascontiguousarray(mul),
                      "derived: search round(mlp_requant_scale * 2^shift)", "v6 fixed-point requant multiplier"))
        extra.append((f"{stage}.mlp_requant_shift", np.ascontiguousarray(shift),
                      "derived: best right-shift for mlp_requant_scale", "v6 fixed-point requant shift"))
...
def main() -> None:
    ...
    fix_extra, fix_triples = _build_mlp_requant_fixed(mlp_extra)  # v6 integer mul/shift for the MLP requant
    fields = fields + mlp_extra + ssm_extra + fix_extra
    ...
    print(f"  added {len(fix_extra)} int32 files: mlp_requant_mul.bin x{len(fix_triples)} + mlp_requant_shift.bin x{len(fix_triples)} (v6)")
    v4._validate_requant(mlp_triples)
    v4._validate_requant(ssm_triples)
    _validate_fixed(fix_triples)
